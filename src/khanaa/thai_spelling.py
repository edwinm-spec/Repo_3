...
class SpellWord:
    ...
    def _find_tone_info(self) -> List[str]:
        ...
        # Handle "no usage" cases for tone 4 with dead syllables
        if (self.tone == 4 and alive_dead == 'dead'):
            # These combinations theoretically exist but are not used in practice
            # Fall back to tone 0 (สามัญ) for dead syllables with tone 4
            tone_info = TONES[0][word_detail]
            # Clear tone mark since we're falling back to tone 0
            return [tone_info[0], '']
        else:
